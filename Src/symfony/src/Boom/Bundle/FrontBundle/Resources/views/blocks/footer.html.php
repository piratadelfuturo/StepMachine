
<a href="https://www.surveymonkey.com/s/7BOOMtest" target="_blank" class="encuesta">Contesta nuestra encuesta de prueba</a>
<footer>
  <ul class="f-cont cf">
    <li class="about">
      <h3><a href="#">Sobre 7Boom</a></h3>
      <p>7boom es una herramienta de socialización a partir de tu opinión. El espacio donde tú ordenas al mundo como tú quieras. El límite es el siete.</p>
      <ul class="bc-site">
        <li><a href="https://www.facebook.com/7boomMx" target="_blank" class="fb-btn">facebook</a></li>
        <li><a href="https://www.twitter.com/7_boom"   target="_blank" class="tw-btn">twitter</a></li>
        <li><p><span>7boom es</span> un sitio de:</p></li>
        <li><a href="#" class="bc-logo">Brutal Content</a></li>
      </ul>
    </li>
    <li class="contact">
      <h3><a href="#">Tu marca en 7Boom</a></h3>
      <p>¿Te interesa anunciarte en 7Boom?</p>
      <p>Contactanos en <a href="mailto:contacto@7boom.mx">contacto@7boom.mx</a></p>
    </li>
    <li class="newsletter">
      <h3><a href="#">Recibe nuestro newsletter</a></h3>
      <form action="http://7boom.us6.list-manage2.com/subscribe/post?u=8e9171246d53eebc71ca63890&amp;id=ef91d2cbb8" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate>
      
      	<input type="text" value="" name="FNAME" class="" id="mce-FNAME" placeholder="Nombre completo">
      	<input type="email" value="" name="EMAIL" class="required email" id="mce-EMAIL" placeholder="Correo electrónico">
      	<div id="mce-responses" class="clear">
      		<div class="response" id="mce-error-response" style="display:none">
      		</div>
      		<div class="response" id="mce-success-response" style="display:none">
      		</div>
      	</div>	
      <input type="submit" value="ENVIAR" name="subscribe" id="mc-embedded-subscribe" class="button">
      </form>
    </li>
  </ul>
  <p id="copyright">Copyright © 2012 · All Rights Reserved · <a href="mailto:contacto@7boom.mx">contacto@7boom.mx</a></p>
</footer>
